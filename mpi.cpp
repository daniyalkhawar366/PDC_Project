...
void printResults(const vector<int>& dist, const vector<int>& parent,
    ...
    set<int> reachable_node_ids; // Track unique reachable node IDs
    ...
    for (size_t i = 0; i < dist.size(); i++) {
        if (dist[i] != INF) {
            ...
            reachable_node_ids.insert(idx_to_id[i]); // Store node ID
    ...
    // Gather all reachable node IDs to rank 0
    int size;
    MPI_Comm_size(MPI_COMM_WORLD, &size);

    // Convert set to vector for MPI communication
    vector<int> local_reachable_ids(reachable_node_ids.begin(), reachable_node_ids.end());
    int local_count = local_reachable_ids.size();

    // Gather counts of reachable node IDs from each process
    ...
    MPI_Gatherv(local_reachable_ids.data(), local_count, MPI_INT,
    ...
    if (rank == 0) {
        ...
        // Compute global total distance and max distance
        // Since distances are synchronized across processes, use rank 0's dist array
        for (size_t i = 0; i < dist.size(); i++) {
            if (dist[i] != INF) {
...
int main(int argc, char** argv) {
    ...
    if (num_vertices > 0) {
        ...
        // SECOND PARTITION: Repartitioning after all updates are complete
        if (rank == 0) {
        ...
        // Final SSSP computation after all updates and repartitioning
        if (rank == 0) {
            cout << "\n===== Final SSSP Computation after All Updates =====" << endl;
        }

        // Reset distances for final computation
